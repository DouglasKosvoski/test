...
class TestTracOSRepository:
    ...
    async def test_find_all_unsynced_workorders(self, mock_db_connection):
        ...
        collection._storage.extend([
            {"number": 1, "status": "created", "title": "Unsynced 1", 
             "description": "Desc", "createdAt": datetime.now(timezone.utc),
             "updatedAt": datetime.now(timezone.utc), "deleted": False, "isSynced": False},
            {"number": 2, "status": "completed", "title": "Synced", 
             "description": "Desc", "createdAt": datetime.now(timezone.utc),
             "updatedAt": datetime.now(timezone.utc), "deleted": False, "isSynced": True},
            {"number": 3, "status": "pending", "title": "Unsynced 2", 
             "description": "Desc", "createdAt": datetime.now(timezone.utc),
             "updatedAt": datetime.now(timezone.utc), "deleted": False},  # No isSynced field
        ])
    ...
    async def test_mark_workorder_as_synced(self, mock_db_connection):
        ...
        collection._storage.append({
            "number": 1, "status": "created", "title": "Test",
            "description": "Desc", "createdAt": datetime.now(timezone.utc),
            "updatedAt": datetime.now(timezone.utc), "deleted": False, "isSynced": False
        })
...
class TestTracOSRepositoryErrorHandling:
    ...
    async def test_save_workorder_connection_error_propagates(self):
        ...
        with patch('integration.system.tracos.repository.get_connection', new_callable=AsyncMock) as mock_conn:
